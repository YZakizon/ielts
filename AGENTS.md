...
- After a pull request is merged, always remove the merged local task branch and any task worktree created for that work. Delete the merged remote task branch only when the user explicitly requests it. Do not delete unrelated branches, worktrees, or remote branches.
- When the user says "let's test this", apply the current task changes to the primary working tree (the "maintree") without changing its branch, then rebuild the local Docker application so the user can test it locally.
